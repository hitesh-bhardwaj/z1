...
export default function BlogPage() {
  ...
  const loadMorePosts = () => {
    setLoadingMore(true);
    setTimeout(() => {
      setVisiblePosts(visiblePosts + 6); // Increase the number of posts to show
      setLoadingMore(false);
    }, 1000); // Simulate a delay for loading more posts
  };
  ...
  const renderedBlogPosts = Blogs.slice(0, visiblePosts).map((post, index) => (
    <div key={index} className="blog-page-grid">
      {/* Render your blog post here */}
    </div>
  ));
